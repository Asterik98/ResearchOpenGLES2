...
void Model::InitModel(char* filename) {
	...
	while (fscanf_s(pFile, "%c", &c) != EOF) {
		if (fscanf_s(pFile, "pos:[%f, %f, %f]; norm:[%f, %f, %f]; binorm:[%*f, %*f, %*f]; tgt:[%*f, %*f, %*f]; uv:[%f, %f];", &verticesData[CounterVertices].pos.x, &verticesData[CounterVertices].pos.y, &verticesData[CounterVertices].pos.z, &verticesData[CounterVertices].norm.x, &verticesData[CounterVertices].norm.y, &verticesData[CounterVertices].norm.z, &verticesData[CounterVertices].uv.x, &verticesData[CounterVertices].uv.y) > 0) {
